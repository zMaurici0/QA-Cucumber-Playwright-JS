...
class CarrinhoPage {
  constructor(page) {
    ...
    this.precoItem1 = page.locator('[data-test="inventory-item-price"]').nth(0).textContent();
    this.precoItem2 = page.locator('[data-test="inventory-item-price"]').nth(1).textContent();
    this.subtotal = page.locator('.summary_subtotal_label').textContent();
  }
  ...
  async verifyTotal(){
    let preco1 = await this.precoItem1
    let preco2 = await this.precoItem2

    preco1 = preco1.replace('$', '')
    preco2 = preco2.replace('$', '')
    let total = parseFloat(preco1) + parseFloat(preco2);

    let subtotalText = await this.subtotal;
